...
export async function fetchRepositoryLabels(token, repository, apiBase, fetcher = fetch) {
...
export async function resolveLabels(value, env, fetcher = fetch) {
    if (value === 'repository') {
        return validateLabels(await fetchRepositoryLabels(input(env, 'github-token', env.GITHUB_TOKEN ?? ''), env.GITHUB_REPOSITORY ?? '', env.GITHUB_API_URL ?? 'https://api.github.com', fetcher));
    }
    ...
    const available = await fetchRepositoryLabels(input(env, 'github-token', env.GITHUB_TOKEN ?? ''), env.GITHUB_REPOSITORY ?? '', env.GITHUB_API_URL ?? 'https://api.github.com', fetcher);
...
function assetName() {
    if (process.platform === 'linux' && process.arch === 'x64')
        return 'jev-linux-x86_64';
    if (process.platform === 'darwin' && process.arch === 'arm64')
        return 'jev-macos-aarch64';
    throw new Error('Jev supports Linux x86_64 and macOS arm64 runners');
}
export async function installJev(version, runnerTemp, fetcher = fetch, releaseBase = 'https://github.com/model-clis/jev/releases/download') {
...
export async function runJev(binary, request, assertion, apiKey, env) {
    const args = ['ask', request];
    if (assertion)
        args.push('--assert', assertion);
    const childEnv = Object.fromEntries(Object.entries(env).filter(([name]) => !name.startsWith('INPUT_') && name !== 'GITHUB_TOKEN' && name !== 'GH_TOKEN'));
    const result = await new Promise((done, fail) => {
        ...
        child.on('close', (code) => {
            if (oversized)
                fail(new Error('Jev response exceeded 1 MiB'));
            else
    ...
    if (!text) {
        if (result.code === 0)
            throw new Error('Jev returned no response');
...
export async function runAction(env = process.env, fetcher = fetch, releaseBase) {
    ...
    try {
        ...
        if (requestFile) {
            ...
            if (!statePath)
                throw new Error('GITHUB_EVENT_PATH or state-file is required');
        ...
        if (result.response !== undefined)
            await writeOutput(env, 'response', result.response);
        return result.code;
    }
    finally {
